...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.channel_independence = False #configs.channel_independence
        self.sparsity_threshold = 0.01
        self.scale = 0.02
        self.embeddings = nn.Parameter(torch.randn(1, self.embed_size))


        embed_size = self.embed_size + configs.node_dim
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.seq_len * embed_size, self.hidden_size),
            nn.LeakyReLU(),
        ...
        self.adapter = nn.Parameter(torch.empty(configs.enc_in, self.embed_size, self.rank))  # [n,D,d]
    ...
    def forecast(self, x_enc):
        ...
        x = x.permute(0, 2, 3, 1)  # [B, N, T, D] -> [B, T, D, N]
        adaptation.append(torch.einsum('btDn,bDdn->btdn', [x, adapter]))  # [B, T, d, N]
        x = torch.cat([x] + adaptation, dim=-2)  # [B, T, H', N]
        x = x.permute(0, 3, 1, 2)  # [B, T, d, N] -> [B, N, T, D‘]
        ## Local adaptation
